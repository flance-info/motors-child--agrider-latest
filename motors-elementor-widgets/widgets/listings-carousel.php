...
$sell_online               = apply_filters( 'stm_me_get_nuxy_mod', false, 'enable_woo_online' );
				...
				while ( $special_query->have_posts() ) :
					...
					if ( empty( $spec_banner ) ) :
						...
						if ( function_exists( 'stm_get_listings_filter' ) ) {
							$labels = apply_filters( 'stm_get_listings_filter', array(), get_post_type( get_the_ID() ), array( 'where' => array( 'use_on_car_listing_page' => true ) ), false );
						} else {
												...
												<div class="image dp-in">
													...
													if ( true === $gallery_hover_interaction && ! wp_is_mobile() ) {
														...
														if ( empty( $thumbs['gallery'] ) || 1 === count( $thumbs['gallery'] ) ) :
															?>
															<img src="<?php echo esc_url( $img[0] ); ?>" data-retina="<?php echo esc_url( $img_2x[0] ); ?>" class="img-responsive" alt="<?php echo esc_attr( apply_filters( 'stm_get_img_alt', '', get_post_thumbnail_id( get_the_ID() ) ) ); ?>">
															<?php
																	...
																	if ( ! empty( $price ) && ! empty( $sale_price ) ) :
																		...
																		<div class="price heading-font discounted-price">
																			...
																			<div class="sale-price">
																				<?php echo esc_attr( apply_filters( 'stm_filter_price_view', '', $sale_price ) ); ?>
																			</div>
																		</div>
																		<?php
																	elseif ( ! empty( $price ) ) :
...
<!--												--><?php //endif; ?>
                                            ...
                                            <div class="car-prices">
                                                <?php if($price): ?>
                                                    <p class="stm-regular-price"><span><?php esc_html_e('Price', 'motors-elementor-widgets'); ?>:</span>
												...
												<?php if($regular_price_description): ?>
													<p class="stm-description-price">
														<span>
														<?php esc_html_e('Leasing price', 'motors-child'); ?>:
														</span>
														<?php /* echo $regular_price_description; */?>														                        <?php echo $sale_price ? apply_filters('stm_filter_price_view', '', ($sale_price * 0.30))  : apply_filters('stm_filter_price_view', '', ($price * 0.30)); ?>
